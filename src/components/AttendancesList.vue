<template>
  <div
    ...
    :key="attendance.id"
    class="bg-white rounded-md shadow-sm px-6 sm:px-8 py-4 sm:py-6 mb-3 flex items-center justify-between"
  >
    <div class="w-1/2 sm:w-1/3 pr-10 flex items-center">
      ...
      <div class="ml-5 flex flex-col w-full">
        <h3 class="truncate font-semibold text-sm 2xl:text-base">
          Attendance for {{ attendance.created_at }}
        </h3>
      </div>
    </div>

    <div
      class="hidden sm:flex items-center py-3 px-6 md:px-10 md:border-r-2 md:border-l-2"
    >
      <svg
