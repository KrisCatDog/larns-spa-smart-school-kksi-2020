<template>
  <nav
    id="nav"
    class="bg-white flex justify-between items-center px-8 sm:px-16 py-5 border-b"
  >
    ...
    <div class="flex items-center">
      <router-link
        :to="{ name: 'Classrooms' }"
        class="text-black-300 font-semibold px-6 hidden sm:flex items-center border-r-2"
      >
        <svg
          width="24"
          height="24"
        ...
        <span class="inline-block pb-1 ml-2 text-sm">Classrooms</span>
      </router-link>
  ...
  <nav
    id="nav-responsive"
    class="-mr-4 flex items-center justify-center sm:hidden fixed left-0 right-0 bottom-0 py-2 px-3 border-t-2 bg-white z-50"
  >
    <router-link
      :to="{ name: 'Classrooms' }"
    ...
    >
      ...
      <span class="inline-block text-xs font-regular">Classrooms</span>
    </router-link>
  </nav>
</template>
