<template>
  <div>
    <nav id="nav" class="flex justify-between items-center px-16 py-5 border-b">
      <img src="./../assets/logo.svg" alt="larns logo" class="w-24" />

      <div class="flex items-center">
        <router-link
          :to="{ name: 'Classrooms' }"
          class="text-black-300 font-semibold px-6 flex items-center border-r-2"
        >
          <svg
            width="24"
            height="24"
          ...
          <span class="inline-block pb-1 ml-2">Classes</span>
        </router-link>

        <div class="ml-6 w-10 h-10 bg-gray-400 rounded-full"></div>
      </div>
    </nav>
  </div>
</template>
